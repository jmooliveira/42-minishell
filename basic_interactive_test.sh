...
MINISHELL="./minishell"
PROMPT="/minishell\$ /d" # put your prompt here without change de "/" and "/d"
PROMPT_EDGE="s/minishell\$ //g" # put your prompt here without change de "s/" and "//g"
MINISHELL_FOLDER=$PWD
...
compare_output() {
    ...
    mini_out=$(echo "$mini_out" | sed "$PROMPT_EDGE" | tr -s ' ' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
...
test_heredoc() {
    ...
    if ! command -v expect &> /dev/null; then
        ...
        echo
        echo "7. cat << \"END\""
        echo "8. Digite \$HOME (não deve expandir)"
        echo "9. Saia com sinal Ctrl-D ou Ctrl-C"
        echo
        echo "10. <<EOF"
        echo "11. Digite algumas linhas"
        echo "12. Digite variáveis de ambiente (deve expandir)"
        echo "13. Saia com sinal Ctrl-D ou Ctrl-C"
        return
    fi
...
main_tests() {
    ...
    run_test_with_exit "cd $MINISHELL_FOLDER" "CD de volta para projeto" false 0
...
stress_tests() {
    ...
    run_test_with_exit "cd ../../.." "CD com múltiplos .." false 0
    run_test_with_exit "cd $MINISHELL_FOLDER" "Volta para projeto" false 0
...
comprehensive_feature_tests() {
    ...
    run_test_with_exit "cd $MINISHELL_FOLDER" "CD volta projeto" false 0
    ...
    run_test_with_exit "env arg1" "Env com argumentos (deve dar erro)" true 1
